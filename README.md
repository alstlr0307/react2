...
## Next.js 이미지 & 폰트 최적화 + Docker 배포 정리

#### Next Steps ➜ Guides

-   애플리케이션에서 CSS를 사용할 수 있는 다양한 방법들을 더 자세히
    알아보세요.

##### Tailwind CSS v3

-   Tailwind CSS v3를 사용하여 더 넓은 브라우저 지원을 위해 Next.js
    애플리케이션을 스타일링할 수 있습니다.

#### Sass

-   Sass를 사용하여 Next.js 애플리케이션의 스타일을 지정할 수 있습니다.

#### CSS-in-JS

-   Next.js에서 다양한 CSS-in-JS 라이브러리를 활용할 수 있습니다.
...
-   Next.js의 `<Image>` 컴포넌트는 HTML `<img>` 요소를 확장한 기능을
    제공합니다.

### 📌 크기 최적화

-   각 기기에 맞게 자동으로 최적 크기의 이미지를 제공하며 WebP 같은 최신
    이미지 형식을 사용합니다.

### 📌 시각적 안정성(CLS 방지)

-   이미지 로딩 시 레이아웃 이동(Layout Shift) 문제를 자동으로
    방지합니다.

### 📌 더 빠른 페이지 로드

-   기본 브라우저 lazy-loading 기능을 사용하며 placeholder 옵션도
    제공합니다.

### 📌 자산(Asset) 유연성

-   원격 서버 이미지도 원하는 크기로 조절해 사용할 수 있습니다.
...
-   WebP는 효율적 압축을 통해 파일 크기를 크게 줄이고 손실/무손실 압축,
    투명도(알파), 애니메이션을 지원합니다.
...
1.  파일 크기 작고 빠른 로딩 속도
2.  데이터 전송 속도 향상
3.  기존 이미지 형식(JPEG/PNG/GIF) 장점 포함
...
-   정적 파일은 `public` 폴더에 저장
-   `/` 로 접근 가능

``` tsx
<Image className="dark:invert" src="/nextjs.png" alt="" width={360} height={218} />
```
...
-   import된 이미지: width/height 자동 계산
-   `/src/images/` 폴더 사용
...
``` tsx
<Image src="https://s3.amazonaws.com/my/profile.png" width={500} height={500} />
```

-   원격 파일 접근 불가 → width/height 직접 입력
-   blurDataURL 옵션 사용 가능
-   fill 옵션으로 영역 채우기 가능
...
``` tsx
const foo = localFont({
  src: '/src/fonts/GamjaFlower-Regular.woff2',
  weight: '400',
  style: 'normal',
})
```
...
-   Docker 지원 모든 플랫폼에 배포 가능
-   Kubernetes 포함
-   Next.js 모든 기능 지원
-   개발 시 Mac/Windows에서는 성능 때문에 local 개발 권장
